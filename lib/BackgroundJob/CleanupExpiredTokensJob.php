...
namespace OCA\SignaturePDF\BackgroundJob;
	...
	protected function run($argument) {
		...
		$this->userManager->callForAllUsers(function (IUser $user) use ($now): void {
			...
			try {
				...
				$this->logger->warning('Could not load app passwords for user while cleaning SignaturePDF tokens', [
			...
			foreach ($tokens as $token) {
				...
				try {
					...
					$this->logger->warning('Could not delete expired SignaturePDF token', [
